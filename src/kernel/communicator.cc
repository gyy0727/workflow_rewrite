...
int Communicator::send_message_sync(struct iovec vectors[], int cnt,
  ...
  if (service) {
    __sync_add_and_fetch(&entry->ref_, 1);
    //*发送完,自然就是keepalive等待下次连接
    timeout = session->keep_alive_timeout();
    switch (timeout) {
    default:
      //*keepalive==0,那就直接关闭
      if (entry && timeout == 0) {
        mpoller_del(entry->sockfd_, this->mpoller);
        entry->state_ = CONN_STATE_CLOSING;
        break;
      }
      mpoller_set_timeout(entry->sockfd_, timeout, this->mpoller);
      // pthread_mutex_lock(&service->mutex);
      std::unique_lock<std::mutex> lock(service->m_mutex);
      if (service->listen_fd >= 0) {
        entry->state_ = CONN_STATE_KEEPALIVE;
        //*将当前连接对应的entry添加到service的active_list中
        list_add_tail(&service->alive_list, &entry->list_);
        //*似乎entry只用于一次session就会销毁
        entry = NULL;
      }
      // pthread_mutex_unlock(&service->m_mutex);
      lock.unlock();
      break;
    }
  } else {
    //*udp
    if (entry->state_ == CONN_STATE_IDLE) {
    ...
    //*等待接收数据
    entry->state_ = CONN_STATE_RECEIVING;
  }
...
int Communicator::send_message_async(struct iovec vectors[], int cnt,
  ...
  entry->write_iov_ = (struct iovec *)malloc(cnt * sizeof(struct iovec));
  //*先将要发送的数据拷贝到entry,因为是异步发送,就肯定得有副本
  if (entry->write_iov_) {
    for (i = 0; i < cnt; i++)
      entry->write_iov_[i] = vectors[i];
  } else
    return -1;

  data.operation = PD_OP_WRITE;
  data.fd = entry->sockfd_;
  //*partial_written主要作用是设置一个定时器,定时器期限内不完成发送就移除当前写任务
  data.partial_written = Communicator::partial_written;
  data.context = entry;
  data.write_iov = entry->write_iov_;
  data.iovcnt = cnt; //*要发送的结构体的数量
  //*获取超时
  timeout = Communicator::first_timeout_send(entry->session_);
  //*idle是udp的状态
  if (entry->state_ == CONN_STATE_IDLE) {
